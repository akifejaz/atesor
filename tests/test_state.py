"""
Aggressive tests for src/state.py — the single source of truth for agent state.

These cover:
  * AgentState construction + mutation helpers (add_error, add_fix_attempt, caches,
    audit trail, command-key collisions).
  ...
  * infer_failure_severity: covers the LOW/HIGH overrides and the broad-MEDIUM bucket.
  * should_escalate: every escalation trigger (max attempts, error loop, fundamental
    blocker, $ cap).
...
class TestInitialState(unittest.TestCase):
    ...
    def test_repo_name_strips_trailing_slash(self):
        state = create_initial_state("https://github.com/foo/bar/")
        self.assertEqual(state.repo_name, "bar")

    def test_custom_max_attempts_honored(self):
        state = create_initial_state("https://github.com/foo/bar", max_attempts=12)
        self.assertEqual(state.max_attempts, 12)

    def test_history_collections_are_independent_instances(self):
        """Regression: dataclass mutable defaults must use field(default_factory)."""
        ...
        self.assertEqual(len(s2.error_history), 0,
...
class TestStateMutation(unittest.TestCase):
    ...
    def test_add_error_updates_all_fields_and_attempt(self):
        err = create_error_record("Permission denied", ErrorCategory.PERMISSION)
    ...
    def test_consecutive_add_error_increments_attempts(self):
        for i in range(4):
    ...
    def test_log_api_call_accumulates_cost(self):
    ...
    def test_command_cache_stores_and_retrieves(self):
    ...
    def test_command_cache_miss_returns_none(self):
        self.assertIsNone(self.state.get_cached_command_result("never-run"))

    def test_command_cache_keyed_by_hash_avoids_collisions(self):
    ...
    def test_log_event_records_agent_and_timestamp(self):
    ...
    def test_get_last_audit_events_limit(self):
        for i in range(10):
            self.state.log_event(f"e{i}", {})
        self.assertEqual(len(self.state.get_last_audit_events(limit=3)), 3)
        self.assertEqual(self.state.get_last_audit_events(limit=3)[-1]["event"], "e9")

    def test_log_agent_decision_writes_decision_record(self):
    ...
    def test_add_build_artifact_records_metadata(self):
        self.state.add_build_artifact("/p/binary", "binary", architecture="RISC-V")
...
class TestErrorLoop(unittest.TestCase):
    def test_not_a_loop_with_fewer_than_three(self):
        state = create_initial_state("https://github.com/a/b")
        for _ in range(2):
            state.add_error(create_error_record("e", ErrorCategory.COMPILATION))
        self.assertFalse(state.is_in_error_loop())

    def test_loop_when_three_consecutive_same_category(self):
        state = create_initial_state("https://github.com/a/b")
        for _ in range(3):
            state.add_error(create_error_record("e", ErrorCategory.COMPILATION))
        self.assertTrue(state.is_in_error_loop())

    def test_mixed_categories_not_a_loop(self):
        state = create_initial_state("https://github.com/a/b")
    ...
    def test_loop_examines_only_last_three(self):
        """Older different errors must not prevent loop detection on the last 3."""
        state = create_initial_state("https://github.com/a/b")
...
class TestClassifyError(unittest.TestCase):
    # (message, expected category) tuples — covers every branch in state.classify_error
    CASES = [
        ...
        # Autotools/configure before COMPILATION (syntax error inside configure)
        ("possibly undefined macro AM_GNU_GETTEXT", ErrorCategory.CONFIGURATION),
        ("configure: syntax error near unexpected token", ErrorCategory.CONFIGURATION),
        ...
        ("directory prefix . does not contain main module", ErrorCategory.CONFIGURATION),
    ...
    def test_all_known_patterns(self):
        for msg, expected in self.CASES:
            with self.subTest(msg=msg):
                self.assertEqual(
                    classify_error(msg), expected,
                    f"classify_error({msg!r}) -> wrong category"
                )

    def test_rate_limit_beats_timeout(self):
        """Regression: 'rate limit ... timeout' must classify as RATE_LIMIT, not NETWORK."""
        self.assertEqual(
    ...
    def test_classify_is_case_insensitive(self):
        ...
        self.assertEqual(classify_error("Permission Denied"), ErrorCategory.PERMISSION)
...
class TestInferFailureSeverity(unittest.TestCase):
    def test_which_probe_is_low(self):
        self.assertEqual(
            infer_failure_severity(ErrorCategory.MISSING_TOOLS, command="which ninja"),
            FailureSeverity.LOW,
        )

    def test_high_categories_always_high(self):
        for cat in [
    ...
    def test_clone_pull_apk_high(self):
        for cmd in [
    ...
    def test_build_failure_medium(self):
        for cat in [
            ErrorCategory.CONFIGURATION,
    ...
    def test_unknown_category_medium(self):
        self.assertEqual(
...
class TestShouldEscalate(unittest.TestCase):
    ...
    def test_no_escalation_on_empty_state(self):
        ok, reason = should_escalate(self._state())
        self.assertFalse(ok)
        self.assertEqual(reason, "")

    def test_escalate_when_max_attempts_reached(self):
    ...
    def test_escalate_on_error_loop(self):
    ...
    def test_escalate_on_fundamental_blocker(self):
        s = self._state()
    ...
    def test_escalate_when_cost_exceeded(self):
        s = self._state()
        s.api_cost_usd = 1.5
        ok, reason = should_escalate(s)
        self.assertTrue(ok)
...
class TestNextAction(unittest.TestCase):
    ...
    def test_plan_first(self):
        self.assertEqual(get_next_action_recommendation(self._state()), Action.PLAN)

    def test_scout_when_task_plan_but_no_build_plan(self):
        s = self._state()
        s.task_plan = TaskPlan(phases=[])
        self.assertEqual(get_next_action_recommendation(s), Action.SCOUT)

    def test_builder_when_pending_with_plan(self):
        s = self._state()
        s.task_plan = TaskPlan(phases=[])
        s.build_plan = BuildPlan(
            build_system="cmake", build_system_confidence=0.9,
            phases=[], total_estimated_duration="1m",
        )
        # default BuildStatus.PENDING
        self.assertEqual(get_next_action_recommendation(s), Action.BUILDER)

    def test_fixer_when_failed_non_dependency(self):
        s = self._state()
        s.task_plan = TaskPlan(phases=[])
        s.build_plan = BuildPlan(
            build_system="cmake", build_system_confidence=0.9,
    ...
    def test_scout_when_failed_dependency(self):
        s = self._state()
        s.task_plan = TaskPlan(phases=[])
        s.build_plan = BuildPlan(
            build_system="cmake", build_system_confidence=0.9,
    ...
    def test_finish_after_success_with_tests(self):
        s = self._state()
        s.task_plan = TaskPlan(phases=[])
        s.build_plan = BuildPlan(
            build_system="cmake", build_system_confidence=0.9,
    ...
    def test_finish_after_success_even_without_tests(self):
    ...
    def test_escalate_short_circuits_decision_tree(self):
        s = self._state()
        s.api_cost_usd = 2.0
...
class TestCommandResult(unittest.TestCase):
    ...
    def test_negative_one_is_failed(self):
...
class TestEnumStability(unittest.TestCase):
    """If any of these names change, callers that read state.to_dict() break."""

    def test_build_status_values(self):
        expected = {
            "PENDING", "PLANNING", "SCOUTING", "BUILDING", "TESTING",
            "FIXING", "SUCCESS", "FAILED", "ESCALATED",
        }
        self.assertEqual({s.value for s in BuildStatus}, expected)

    def test_error_category_values_complete(self):
        # These are referenced by classify_error + should_escalate + supervisor routing
        for cat_name in [
            "UNKNOWN", "DEPENDENCY", "COMPILATION", "LINKING", "ARCHITECTURE",
            "NETWORK", "RATE_LIMIT", "CONFIGURATION", "MISSING_TOOLS",
            "PERMISSION", "DISK_SPACE",
            "LICENSE_INCOMPATIBLE", "REQUIRES_HARDWARE", "ARCHITECTURE_IMPOSSIBLE",
        ]:
            self.assertTrue(hasattr(ErrorCategory, cat_name),
                            f"Missing ErrorCategory.{cat_name}")

    def test_agent_role_values(self):
        expected = {"planner", "supervisor", "scout", "builder", "fixer",
                    "summarizer", "agent"}
...
class TestSerialization(unittest.TestCase):
    ...
    def test_to_dict_serializes_nested_enums(self):
        s = create_initial_state("https://github.com/x/y")
        s.add_error(create_error_record("permission denied"))
        d = s.to_dict()
        # error_history is a list of dicts and ErrorCategory inside must serialize
        self.assertEqual(len(d["error_history"]), 1)
        self.assertIn("PERMISSION", d["error_history"][0]["category"])

    def test_save_to_json_writes_valid_json(self, ):
        s = create_initial_state("https://github.com/x/y")
