...
def _load(rel_path: str, name: str):
    """Load a hyphenated/loose script as a module (its name isn't a
    valid Python identifier, so import_module can't reach it)."""
...
class TestApplyShard(unittest.TestCase):
            ...
            for total in [1, 2, 3, 4, 7]:
                ...
                with self.subTest(n=n, total=total):
                    ...
                    for i in range(total):
                        rebuilt.extend(
                            batch_test._apply_shard(pkgs, i, total)
                        )
    ...
    def test_shard_sizes_balanced_within_one(self) -> None:
        ...
        pkgs = self._pkgs(172)
        sizes = [
            len(batch_test._apply_shard(pkgs, i, 4)) for i in range(4)
        ]
    ...
    def test_shard_count_exceeds_packages(self) -> None:
        ...
        pkgs = self._pkgs(3)
        sizes = [
            len(batch_test._apply_shard(pkgs, i, 5)) for i in range(5)
        ]
