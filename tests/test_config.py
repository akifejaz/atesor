...
class TestConfigPaths(unittest.TestCase):
    ...
    def test_subdirs_under_workspace(self):
        for d in [config.OUTPUT_DIR, config.REPOS_DIR, config.CACHE_DIR, config.LOGS_DIR]:
            with self.subTest(d=d):
                self.assertTrue(d.startswith(config.WORKSPACE_ROOT), f"{d} not under workspace")
    ...
    def test_docker_detection_false_on_normal_host(self):
    ...
    def test_dockerenv_file_triggers_detection(self):
