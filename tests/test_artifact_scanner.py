"""Tests for src/artifact_scanner.py — architecture detection and build verification."""
...
def _ok(stdout=""):
    return SimpleNamespace(success=True, failed=False, stdout=stdout, stderr="",
                           exit_code=0, command="", duration_seconds=0.0,
                           timestamp="")
...
class TestDetectArchitecture(unittest.TestCase):
    ...
    CASES = [
        ("ELF 64-bit LSB pie executable, UCB RISC-V, RVC, double-float ABI", "RISC-V"),
    ...
    def test_detect_each_architecture(self):
...
class TestVerifyBuildSuccess(unittest.TestCase):
    ...
    def test_riscv_artifacts_succeed(self):
        sc = self._scanner_with([
            {"filepath": "/build/a", "type": "binary", "architecture": "RISC-V"}
        ])
        ok, msg = sc.verify_build_success()
        self.assertTrue(ok)
        self.assertIn("RISC-V", msg)

    def test_x64_only_artifacts_fail(self):
        sc = self._scanner_with([
    ...
    def test_mixed_arches_with_riscv_succeed(self):
        # If even one RISC-V artifact is found, build counts as success
        sc = self._scanner_with([
            {"filepath": "/build/a", "type": "binary", "architecture": "RISC-V"},
            {"filepath": "/build/b", "type": "binary", "architecture": "x64"},
        ])
        ok, _ = sc.verify_build_success()
        self.assertTrue(ok)

    def test_unknown_arch_artifacts_fail(self):
        sc = self._scanner_with([
...
class TestGetSummary(unittest.TestCase):
    def test_summary_counts_by_type_and_arch(self):
        sc = ArtifactScanner("/tmp/build")
        sc.artifacts = [
        ...
        self.assertEqual(s["by_type"], {"binary": 2, "library_static": 1, "library_shared": 1})
        self.assertEqual(s["by_arch" if "by_arch" in s else "by_architecture"],
                         {"RISC-V": 3, "x64": 1})
...
class TestScanIntegration(unittest.TestCase):
    """Mock execute_command to verify the scan() pipeline calls the right helpers."""
    ...
    def test_scan_calls_find_and_file(self):
        ...
        def fake_exec(cmd, cwd=None, **kw):
            ...
            if cmd.startswith("file "):
                return _ok("/tmp/build/foo: ELF 64-bit LSB executable, UCB RISC-V")
            if "stat" in cmd:
                return _ok("12345")
            return _ok("")

        with mock.patch("src.artifact_scanner.execute_command", side_effect=fake_exec):
            artifacts = sc.scan()
    ...
    def test_scan_handles_no_artifacts(self):
        sc = ArtifactScanner("/tmp/build")
        with mock.patch("src.artifact_scanner.execute_command", return_value=_ok("")):
            artifacts = sc.scan()
