...
class TestPackageBuild(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        with open(
            os.path.join(self.repo_path, "internal", "x.go"), "w"
        ) as f:
    ...
    def test_manifest_contents(self) -> None:
        ...
        self.assertEqual(
            m["repo_url"], "https://github.com/owasp-amass/amass"
        )
    ...
    def test_missing_repo_dir_raises(self) -> None:
        with self.assertRaises(FileNotFoundError):
            self._build(repo_path=os.path.join(self.root, "does", "not", "exist"))
    ...
    def test_logs_included_at_zip_root(self) -> None:
        ...
        zp = self._build(
            agent_log_path=agent_log, batch_log_path=batch_log
        )
        with zipfile.ZipFile(zp) as zf:
            names = set(zf.namelist())
        ...
        self.assertEqual(
            ...
            sorted(
                [f"{self.repo_name}.log", f"agent_{self.repo_name}.log"]
            ),
        )
