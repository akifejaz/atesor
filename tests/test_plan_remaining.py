...
plan_remaining = _load(
    ".github/scripts/plan-remaining.py", "plan_remaining_mod",
)
missing_pkgs = _load(
    ".github/scripts/missing-pkgs.py", "missing_pkgs_pr_mod",
)
slice_pkgs = _load(
    ".github/scripts/slice-pkgs.py", "slice_pkgs_mod",
)
...
class TestReleasedNames(unittest.TestCase):
    ...
    def test_empty_input(self) -> None:
        self.assertEqual(
            plan_remaining._released_names([], "alpine"), set(),
        )
        self.assertEqual(
    ...
    def test_unknown_platform_not_matched(self) -> None:
        names = plan_remaining._released_names(
            ["foo-20260601-150000-windows.zip"], "alpine",
        )
...
class TestComputePlan(unittest.TestCase):
    ...
    def test_preserves_declared_order(self) -> None:
        ...
        remaining, total, groups = plan_remaining.compute_plan(
            declared, released, group_size=50,
        )
...
class TestMissingPkgsPolymorphicLoader(unittest.TestCase):
    ...
    def test_loads_dict_schema(self) -> None:
        ...
        try:
            self.assertEqual(
                missing_pkgs._load_package_names(path), ["a", "b"],
            )
        finally:
            os.unlink(path)
    ...
    def test_loads_string_schema(self) -> None:
        ...
        try:
            self.assertEqual(
                missing_pkgs._load_package_names(path), ["x", "y", "z"],
            )
        finally:
            os.unlink(path)
...
class TestPlanRemainingWritesArtifact(unittest.TestCase):
    ...
    def test_roundtrip(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                missing_pkgs._load_package_names(out), ["a", "b", "c"],
            )
